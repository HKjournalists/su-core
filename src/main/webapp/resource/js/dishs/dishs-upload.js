$(document).ready(function(){
    ...
    /* 加载图片 */
	 upload.setAttachments = function(attachments,clearBtnId,listViewId,templateId){
		 ...
		 if(attachments!=null&&attachments.length>0){
			 ...
			 $("#"+clearBtnId).show();
			 for(var i=0;i<attachments.length;i++){
			     dishsno.dishsNofiles.push(attachments[i]);
	...
	/*删除指定的图片*/
	upload.removeRes=function(attachments,resId,listViewId,clearBtnId){
		var dataSource = new kendo.data.DataSource();
		if(attachments){
			if(resId!=null){
				for(var i=0; i<attachments.length; i++){
	...
	/* 清空流通许可证资源  */
	dishsno.clearDishsNoFiles = function(){
		 var listId=[];
		 var count = 0;
		 for(var i=0;i<dishsno.dishsNofiles.length;i++){
		 	if(dishsno.dishsNofiles[i].id != null){
		 		listId.push(dishsno.dishsNofiles[i].id);
		 	}else{
				 count++;
			}
		 }
		 dishsno.dishsNofiles.length=count;
		 $("#disDishsNOFileListView").data("kendoListView").dataSource.data([]);
