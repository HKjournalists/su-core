$(function() {
    ...
    function initialize(){
    	lookLoadOutBusInfo();
    	CONFIRM = initKendoWindow("CONFIRM_COMMON","300px", "130px", "批发提示！", false,true,false,["Close"],null,"");
        ...
        buildGridWioutToolBar("prodcut_detail",PRODUCT_DETAIL_COLUMNS,PRODUCT_DETAIL_DS,"400",true);//已确认批发的商品
    ...
    relation.openAddBus = function(){
        upload.buildGridWioutToolBar("bus_grid",BUS_COLUMNS,getStoreDS2("",""),"245");
    ...
    relation.openBarcode = function(){
    	if(!PRODUCT_GRID){
            upload.buildGridWioutToolBar("product_grid",PRODUCT_COLUMNS,getStoreDS("",""),"350",false);
    		PRODUCT_GRID = $("#product_grid").data("kendoGrid");
    		PRODUCT_GRID.bind("change", grid_change);
    	}else{
    	 	$("#name").val("");
    	 	$("#barcode").val("");
    	 	PRODUCT_GRID.setDataSource(getStoreDS("",""));
    	}
	     /* 清空勾选择的CheckBox */
    ...
    function returnProductInfo(status){
         ...
         if(pArr!=null&&pArr.length>0){
         	for(var x= 0;x<pArr.length;x++){
                ...
                var productionDate = pArr[x].productionDate;
                if(productionDate==null||productionDate==""){//判断是否输入了生产日期
                    fsn.initNotificationMes("请输入生产日期！",false);
                    return false;
                    break;
                }
                ...
                var num = pArr[x].returnCount;
                if(num<=0){
         			fsn.initNotificationMes("批发数量须大于0！",false);
         			return;
         		}
    ...
    relation.check = function(){
	 	...
	 	PRODUCT_GRID.setDataSource(getStoreDS(name,barcode));
    ...
    relation.checkRelation = function(){
	 	...
	 	BUS_GRID.setDataSource(getStoreDS2(busname,lic));
	 	BUS_GRID.refresh();
	 };
    ...
    function buildGridWioutToolBar(id,columns,ds, height,isEdit){
        $("#" + id).kendoGrid({
  ...
  //已经选择的商品
    PRODUCT_DETAIL_COLUMNS = [ {field: "id",title: fsn.l("Id"),width: 1,filterable: false},
                               ...
                               {field: "batch",width: 75, title: fsn.l("批次"),filterable: false},
                                ...
                                {field: "reportId",width: 20, title: fsn.l("报告ID"),hidden:true,
                                    template: function (e) {
                                        var tag = "<label id='reportId'></label>";
                                        return tag;
                                    }
                                },
                               {
    ...
    //选择供销关系--数据
    function getStoreDS2(busname,lic){
		 if(busname!=""){
    ...
    //选择商品--数据
    function getStoreDS(name,barcode){
		 if(name!=""){
