$(function(){
	...
	function initialize(){
			...
			transport: {
				read : {
	                ...
	                url : function(options){
						return fsn.getHttpPrefix() +"/tzAccount/view/list/"+ options.page + "/" + options.pageSize + "?number="+number+"&licOrName="+licOrName+"&status=0";
					},
	                dataType : "json",
	 ...
	 wholesale.checkUrl = function(id){
	    	window.location.href = "addWholesale.html?id=" + id;
	 };
	...
	//显示字段
	 wholesale.columns = [
	               {field: "id", title: "编号", width:"5%",filterable: false,editable: false},
	               {field:"inBusName", title:"购货商名称", width:"20%",filterable: false,editable: false},
				   ...
				   {field: "outBusName", title: "供应商名称", width:"20%",filterable: false,editable: false},
				   {field:"",title:fsn.l("状态"), width: "10%",template:function(dataItem){
	               		var status=dataItem.outStatus;//批发状态
	               		var retStatus=dataItem.returnStatus;//退货状态
	                   	 if(status==1&&retStatus==0){
	                   		 return "<span >已确认</span>";
	                      }else if(status==0&&retStatus==0){
		                     return "<span style='color:red'>未确认</span>";
	                      }else if(status==0&&retStatus==1){
		                     return "<span style='color:red'>被退回</span>";
	                      }
					   	}
					 },
