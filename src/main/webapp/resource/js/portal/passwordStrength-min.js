(function(a) {
	a.fn.passwordStrength = function(b) {
		...
		this.each(function() {
			var d = a(this), e = 0, c = false, f = a(this).parent()
					.find(".passwordStrength");
			d.bind("keyup blur", function() {
				e = a.fn.passwordStrength.ratepasswd(d.val(), b);
				e >= 0 && c == false && (c = true);
				f.find("span").removeClass();
				if (e < 35 && e >= 0) {
					f.find("span:first").addClass("thinStrength");
				} else {
					if (e < 60 && e >= 35) {
						f.find("span:lt(2)").addClass("midStrength");
					} else {
						if (e >= 60) {
							f.find("span:lt(3)").addClass("highStrength");
						}
					}
				}
				if (c
						&& (d.val().length < b.minLen || d.val().length > b.maxLen)) {
