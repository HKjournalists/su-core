$(document).ready(function(){
	...
	fsn.src = "http://qa.fsnrec.com/portal/guid/unit/unit_market_";//引导原地址
    ...
    business_unit.initialize = function(){
		...
		var placeDs=[{id:1,label:"综合农贸市场"},{id:2,label:"批发市场"},{id:3,label:"商场"},{id:4,label:"超市"}]
		business_unit.bulidDropDownListByDS("placeType","label","id",placeDs);
    ...
    business_unit.initView = function(){
        isNew = false;
        $.ajax({
            url: portal.HTTP_PREFIX + "/business/getMarketByOrg",
            type: "GET",
            dataType: "json",
            async:false,
            success: function(returnValue){
            	var market = returnValue.data;
				business_unit.marketId=market.id;
                business_unit.setValue(market.business, "liutong");
				business_unit.setMarketValue(market);
            }
        });
    };
    ...
    business_unit.createInstance = function(){
        ...
        var distribution = {
				...
				toleranceTime : $("#licenseDate").data("kendoDatePicker").value(),
				manageType : $("#busRunType").val().trim(),
				manageProject : $("#busRunProject").val().trim(),				
        	};
			var districtAry = new Array();
			districtAry.push({id:$("#permitLicenseChange").data("kendoDropDownList").value()});
        var subBusiness = {
                id: business_unit.id,
				district:districtAry,
                name: $("#name").val().trim(),
			...
			var model={
				id:business_unit.marketId,
				business:subBusiness,
				administrative: {
					id: $("#belongAreaGroupId").val()
				},
				office: {
					id: $("#authoritiesGroupId").val()
				},
				placeType: {
					id: $("#placeType").val()
				},
				...
				territoryType: {
					id: $("#busDistricType").val()
				},
				prepositionQS:$("#prepositionQS").val(),
...
//        return subBusiness;
    ...
    business_unit.save = function(){
        ...
        $.ajax({
            url: portal.HTTP_PREFIX + "/business/business-market",
            type: "PUT",
            dataType: "json",
            ...
            success: function(returnVal){
            	...
            	if(returnVal.result.status == "true"){
			 		lims.initNotificationMes("交易市场信息修改成功", true);
                    ...
                    business_unit.setLogoAttachments(returnVal.data.logoAttachments);
			 		...
			 		lims.initNotificationMes("交易市场信息修改失败", false);
                }
            }
        });
    };
...
//    	subBusiness = business_unit.createInstanceGZLiutong(subBusiness);
        // 3.保存并上传
        $("#winMsg").html("正在上传交易市场信息，请稍候....");
        $("#k_window").data("kendoWindow").open().center();
        $.ajax({
            url: portal.HTTP_PREFIX + "/business/uploadMarketToFdms",
            type: "POST",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(subBusiness),
            success: function(returnVal){
            	$("#k_window").data("kendoWindow").close();
            	if(returnVal.result.status == "true"){
			 		lims.initNotificationMes("交易市场信息上传成功", true);
                }
                else {
			 		lims.initNotificationMes("交易市场信息上传失败", false);
                }
            }
        });
    };
    ...
    business_unit.validateMyDate = function(){
		...
		if(telephone==null||telephone.length<1){
			lims.initNotificationMes('请填写交易市场基本信息中的联系电话！',false);
			business_unit.wrong("telephone","text");
			return false;
		}
		var barea = $("#belongAreaGroupName").val().trim();
		if(barea == null || barea.length<1) {
			lims.initNotificationMes('请选择交易市场基本信息中的所属区划！',false);
			business_unit.wrong("belongAreaGrouBtn","select");
			return false;
		}
		var author = $("#authoritiesGroupName").val().trim();
		if(author == null || author.length<1) {
			lims.initNotificationMes('请选择交易市场基本信息中的管辖食药监机关！',false);
			business_unit.wrong("authoritiesGrouBtn","select");
			return false;
		}
		if(!$("#permitLicenseChange").data("kendoDropDownList").value()){
			lims.initNotificationMes('请选择食品流通许可证信息中的换证情况！',false);
			business_unit.wrong("permitLicenseChange","select");
			return false;
		}
		if(!$("#busDistricType").data("kendoDropDownList").value()){
			lims.initNotificationMes('请选择交易市场基本信息中的地区类型！',false);
			business_unit.wrong("busDistricType","select");
			return false;
		}
		if(!$("#busAreaType").data("kendoDropDownList").value()){
			lims.initNotificationMes('请选择交易市场基本信息中的地域类型！',false);
			business_unit.wrong("busAreaType","select");
			return false;
		}
		if(!$("#placeType").data("kendoDropDownList").value()){
			lims.initNotificationMes('请选择交易市场基本信息中的场所类型！',false);
			business_unit.wrong("placeType","select");
			return false;
		}
        return true;
    };
	...
	business_unit.setMarketValue = function(dataItem){
		if(dataItem == null) return;
		if(dataItem.administrative!=null){
			$("#belongAreaGroupName").val(dataItem.administrative.name);
			$("#belongAreaGroupId").val(dataItem.administrative.id);
		}else{
			$("#belongAreaGroupName").val("");
			$("#belongAreaGroupId").val("");
		}
		if(dataItem.office!=null){
			$("#authoritiesGroupName").val(dataItem.office.name);
			$("#authoritiesGroupId").val(dataItem.office.id);
		}else{
			$("#authoritiesGroupName").val("");
			$("#authoritiesGroupId").val("");
		}
		if(dataItem.business.distribution!=null){
			$("#busRunType").val(dataItem.business.distribution.manageType);
			$("#busRunProject").val(dataItem.business.distribution.manageProject);
			$("#licenseDate").data("kendoDatePicker").value(dataItem.business.distribution.toleranceTime);
		}else{
			$("#busRunType").val("");
			$("#busRunProject").val("");
			$("#licenseDate").data("kendoDatePicker").value("");
		}
		if (dataItem.placeType != null) {
			$("#placeType").data("kendoDropDownList").value(dataItem.placeType.id);
		}else{
			$("#placeType").data("kendoDropDownList").value(1);
		}
		if (dataItem.areaType != null) {
			$("#busAreaType").data("kendoDropDownList").value(dataItem.areaType.id);
		}else{
			$("#busAreaType").data("kendoDropDownList").value(1);
		}
		if (dataItem.territoryType != null) {
			$("#busDistricType").data("kendoDropDownList").value(dataItem.territoryType.id);
		}else{
			$("#busDistricType").data("kendoDropDownList").value(1);
		}
		$("#prepositionQS").val(dataItem.prepositionQS);
		$("#buildName").val(dataItem.buildName);
		$("#note").val(dataItem.note);
		if (dataItem.business.district != null&&dataItem.business.district!="") {
			$("#permitLicenseChange").data("kendoDropDownList").value(dataItem.business.district[0].id);
		}
	}
	...
	business_unit.validateGZLiutongField = function(){
		...
		tmpId = $("#busDistricType").data("kendoDropDownList").value();
		if(!tmpId){
			lims.initNotificationMes('请选择食品流通许可证信息中的地区类型！',false);
			return false;
		}
		tmpId = $("#busAreaType").data("kendoDropDownList").value();
		if(!tmpId){
			lims.initNotificationMes('请选择食品流通许可证信息中的地域类型！',false);
			return false;
		}
		tmpId = $("#busRunStatus").data("kendoDropDownList").value();
		if(!tmpId){
			lims.initNotificationMes('请选择食品流通许可证信息中的营业状态！',false);
			return false;
		}
		var barea = $("#belongAreaGroupName").val().trim();
		if(barea == null || barea.length<1) {
