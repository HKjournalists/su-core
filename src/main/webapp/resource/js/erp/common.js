$(function(){
	...
	common.initComponent = function(e){
		...
		$("#page_title").html("首页 > " + common.SIMPLE_MODEL_NAME + "管理");
	};
	...
	common.add = function(dailogid, clearFromFun, confrimFun){
		var addWin = eWidget.getWindow(dailogid,common.SIMPLE_MODEL_NAME, common.realObj.win_width == null ? null:common.realObj.win_width);
		clearFromFun();
		addWin.data("kendoWindow").title("新增"+common.SIMPLE_MODEL_NAME);
			...
			//不影响其他模块的修改。判断是如果没有这个字段就自动到else了。所以不会影响其他的模块的
				if(item != null) {
					...
					if(item.outOfBillState!=null && (item.outOfBillState == "已交付" || item.outOfBillState == "已发货")) {
						...
						addWin = eWidget.getWindow(dailogid,common.SIMPLE_MODEL_NAME, common.realObj.win_width == null ? null:common.realObj.win_width);					
