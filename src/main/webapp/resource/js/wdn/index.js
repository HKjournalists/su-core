$(function() {
	...
	wdn.initMenu = function(){
		...
		$("div.btn-group:first button.btn-orderBtn").on("click",function(){
			...
			/*$("div.btn-group button.btn-orderBtn").removeClass('wdn-k-state-hover');
			$(this).attr("class","btn btn-orderBtn wdn-k-state-hover");

			var className = $(this).children().eq(0).attr("class");
			if(className=="glyphicon glyphicon-arrow-down"){
				$(this).children().eq(0).attr("class","glyphicon glyphicon-arrow-up");
			}else{
				$(this).children().eq(0).attr("class","glyphicon glyphicon-arrow-down");
			}*/
		...
		//selectCode = window.sessionStorage.getItem("selectCode");
		//if(selectText!=""&&null!=selectText){
		//	$(".dropdown-toggle")[0].innerHTML=selectText+"&nbsp<span class='caret'></span>";
		//}
	}
	...
	wdn.initListView = function(){
		...
		$("#pop").kendoTooltip({
			content: kendo.template($("#pop_template").html()),
			//width: 400,
			//  height: 200,
			width: 300,
			showOn: "click",
				...
				if(wdn.popGrid==null){
					wdn.popGrid = $("#popGrid").kendoGrid({
						...
						columns: [ {
							...
							headerAttributes: {
								"class": "k-menu",
								style: "font-size: 15px;background:#FFFFFF;color:#0f941a"
							},
							headerTemplate: "最新加入图书",
							template: kendo.template($("#shoop-template").html())
						}],
						height:200
						// selectable: true,
					}).data("kendoGrid");
