	...
	$.ajax({
		...
		success: function(returnValue){
			if (returnValue.result.status == "true") {
				...
				portal.setProAttachmentsFromViewProduct(returnValue.data.proAttachments); //show 产品图片
 ...
 */
portal.editCustomers = function(proId, barcode){
	 portal.barcode = barcode;
	 ...
	 $.ajax({
		   ...
		   success: function(returnValue) {
			  	 if(returnValue.result.status == "true"){
			  		...
			  		$("#customerSelectInfo").data("kendoMultiSelect").value(returnValue.data.split(","));
			  	 }
		   }
	 });
};
 ...
 */
portal.deleteProduct = function(proId){
	 $.ajax({
		   url: portal.HTTP_PREFIX + "/erp/initializeProduct/" + proId,
		   type: "DELETE",
		   dataType: "json",
		   async:false,
		   contentType: "application/json; charset=utf-8",
 ...
 * @author tangxin
 */
portal.setProAttachmentsFromViewProduct = function(dataSource){
