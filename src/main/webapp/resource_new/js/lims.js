...
//    			stacking: "down",z-index: 10003;
  ...
  //初始化easy预览报告的检测项目,根据reportID查询,
	...
	//刷新easy预览报告是检测项目grid中的数据
	lims.easyReadItemsDs = function(repID){
		if(!repID){ repID=0; }
		if(lims.easyTestItemDS){
			lims.easyViewReportID = repID;
			lims.easyItemFirstPageFlag = 1;
			lims.easyTestItemDS.read();
		}
	};
	...
	//给easy的BusUnit赋值
	lims.setEasyBusunitValue = function(busUnit){
  ...
  //展示进口食品报告信息
    lims.setImpProTestInfo=function(impProTestResult){
    	...
    	$.ajax({
			...
			success:function(returnValue){
				if(returnValue.result.status == "true"){
					//预览时如果是进口食品则展示对应信息
					if(returnValue.data.impProTestResult!=null){
						lims.setImpProTestInfo(returnValue.data.impProTestResult);
					}else{
						 $("#sanPdfRepAtt_tr").hide();
						 $("#sanNo_tr").hide();
					}
