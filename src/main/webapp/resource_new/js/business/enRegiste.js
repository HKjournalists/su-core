 $(document).ready(function() {
	 ...
	 enRegiste.updateSignFlag = function(){
	     ...
	     $("#k_window").data("kendoWindow").open().center();
		 $.ajax({
				url:portal.HTTP_PREFIX + "/business/updateSignStatus/"+busName + "/" + signFlag,
				type:"POST",
				dataType:"json",
				contentType: "application/json; charset=utf-8",
				success:function(data){						
					 	if(data.result.success){
					 		$("#k_window").data("kendoWindow").close();
					 		lims.initNotificationMes("企业签名状态修改成功!", true);
					 	}else{
					 		$("#k_window").data("kendoWindow").close();
					 		lims.initNotificationMes("修改企业签名状态时出现后台异常,修改失败！", false);
					 	}
				}
		});
	 };
