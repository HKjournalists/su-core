...
if(st_customer.SIMPLE_TYPE == 3 || isNaN(st_customer.SIMPLE_TYPE)) {
	...
	st_customer.columns = [
	                       ...
	                       {title:"操作",width:operWinth,
	            			      template:function(e){
	            					var tag="<a onclick='return fsn.st_customer.views(" + e.id + ",\"" + e.name + "\",\"" + (e.license?e.license.licenseNo:"")+ "\",\"" + (e.license?e.license.licenseNo:"") + "\",\"" + (e.diyType?e.diyType.name:"") + "\",\"" + e.note + "\")' " +
...
st_customer.initRequiredAreas = function(){
	...
	st_customer.
	cityCombox = $("#ccity").kendoComboBox({
