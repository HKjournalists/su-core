...
//			$("#searchAdd").hide();
	...
	st_customer.addCustomer = function(e,name,lincesNo,typeId,typeName,note,status){
		...
		if(status!=undefined && status == 0){
			...
			_id = e;_name=name;
			_lincesNo = lincesNo;
			_typeId = typeId==null||typeId=='null'||typeId==''?"请选择..":typeId;
			_typeName = typeName==null||typeName=='null'||typeName==''?"请选择..":typeName;
			_note=note;
		}else{
			var tr = $(e.target).closest("tr");
			var data = this.dataItem(tr);
			...
			_typeId = data.diyType.id==null?"-1":data.diyType.id;
			_typeName = data.diyType.name==null?"请选择..":data.diyType.name;
			_note=data.note;
		...
		$("#confirm").bind("click",function(){
			...
			$("#confirm_yes_btn").click(function(){
			  ...
			  // 刷新页面
			   if(status == 0){
			    _type = 0;
			    st_customer.windowSaveConfrim(zone,_type);
			   }else{
				    zone = "searchContext";
					if(_type==1){
						zone ="soldContext";
					...
					st_customer.windowUpdateConfrim(zone,_type);
			   }
