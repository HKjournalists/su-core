$(function(){
	 ...
	 */
	upload.buildUpload = function(id, attachments, msg, flag,isMultiple){
		var selectLocal="";
		if(id=="upload_head_files"){
			selectLocal=fsn.l("upload_Certification");
		}else if(id=="upload_health_files"){
			selectLocal=fsn.l("upload_Health");
		}else if(id=="upload_qualification_files"){
			selectLocal=fsn.l("upload_Qualification");
		}else if(id=="upload_honor_files"){
			selectLocal=fsn.l("upload_Honor");
		}else{
            ...
            success: function(e){
		             ...
		             if (e.operation == "upload") {
		                 ...
		                 attachments.push(e.response.results[0]);
		               	 if(flag == "IMG"){
		               		 $("#"+msg).html("文件识别成功，可以保存!</br>(建议上传照片不超过1M!可支持文件格式：.png .bmp .jpeg .jpg )");
		                 }else if(flag == "PDF"){
