$(function(){
            ...
            read: {
            	url : function(options){
            		if(options.filter){
            			...
            			return portal.HTTP_PREFIX + "/testReport/canPublished/" + options.page + "/" + options.pageSize + "/" + configure + "/3";
            		}
					return portal.HTTP_PREFIX + "/testReport/canPublished/" + options.page + "/" + options.pageSize + "/3";
				},
                dataType : "json",
            ...
            read: {
            	...
            	url : function(options){
            		if(options.filter){
            			var configure = filter.configure(options.filter);
						configure = encodeURIComponent(configure);
            			return portal.HTTP_PREFIX + "/testReport/getAllBackRep/" + options.page + "/" + options.pageSize + "/" + configure + "/true";
            		}
            		return portal.HTTP_PREFIX + "/testReport/getAllBackRep/" + options.page + "/" + options.pageSize + "/true";
            	},
                dataType : "json",
            ...
            read: {
            	url : function(options){
            		if(options.filter){
            			var configure = filter.configure(options.filter);
						configure = encodeURIComponent(configure);
            			return portal.HTTP_PREFIX + "/testReport/byuser/" + options.page + "/" + options.pageSize+"/1/"+ configure + "/true";
            		}
					return portal.HTTP_PREFIX + "/testReport/byuser/" + options.page + "/" + options.pageSize + "/1/true";
				},
                dataType : "json",
                contentType : "application/json"
            }
	...
	root.hasPubColumn = [	
		                   ...
		                   { field: "status", title:"状态", width: 30, filterable: false, template: function(dataItem) {
                                   ...
                                   switch (dataItem.publishFlag){
		                    		...
		                    		return "<strong style='color:#05bf73;'>" + reportStatue + "</strong>";                    		 
		                     }},
		                   { command: [
		                     {name:"review",
	...
	root.column = [	
                   ...
                   { field: "testType", title:"检测类型", width: 40, template: function(dataItem) {
                    	 if(dataItem.signFlag=="未签名" && dataItem.testType=="企业自检"){
                      		 return "<strong style='color:#f2462d;'>" + dataItem.testType + "</strong>";
                    	 }else{
                   ...
                   { field: "signFlag", title:"签名状态", width: 25, filterable: false, template: function(dataItem) {
                  	 if(dataItem.signFlag=="已签名"){
                  		 return "<strong style='color:#f2462d;'>" + dataItem.signFlag + "</strong>";
	...
	root.publishToFSN = function(){
		if(root.busUnit.signFlag && root.currentReport.testType == "企业自检"){
			if(root.currentReport.signFlag == "未签名" || !root.currentReport.signFlag){
		...
		if(root.busUnit!=null&&root.busUnit.type=="流通企业"){
	...
	// 获取当前企业类型
	root.getCurrentBusinessType = function(){
		$.ajax({
			...
			success:function(returnValue){
				if(returnValue.result.status == "true"){
					root.busUnit = {
							...
							signFlag:returnValue.data.signFlag,
					};
				}
			}
		}); 
