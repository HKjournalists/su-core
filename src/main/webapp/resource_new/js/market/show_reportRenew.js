$(function(){
	...
	root.initialize=function(){
		root.buildGrid("undispose_grid", root.setColumn(0), root.loadApplyReportTimesDS(0), "");
		root.buildGrid("disposeing_grid", root.setColumn(1), root.loadApplyReportTimesDS(1), "");
		root.buildGrid("disposed_grid", root.setColumn(2), root.loadApplyReportTimesDS(2), "");
    ...
    /* 加载GRID 的datasource */
    root.loadApplyReportTimesDS = function(status){
                ...
                read: {
                    url: function(options){
                        if (options.filter) {
                            ...
                            return portal.HTTP_PREFIX + "/test/applyTimes/getApplyReportTimes/" + status + "?configure=" + configure + "&page=" + options.page + "&pageSize=" + options.pageSize;
                        }
                        return portal.HTTP_PREFIX + "/test/applyTimes/getApplyReportTimes/" + status + "?page=" + options.page + "&pageSize=" + options.pageSize;
                    },
                    dataType: "json",
    ...
    root.setColumn = function(status){
        ...
        if(status==2){
            column =[
                ...
                {field: "applyDate",title: "申请时间",width: 55,template: '#= (new Date(applyDate)).format("YYYY-MM-dd")#'}, 
    ...
    root.addReportBaseInfo = function(barcode){
       ...
       //有权限：跳到录报告页面
        if (root.isPower) {
            /* 传条形码 */
            $.cookie("product_barcode", JSON.stringify(barcode), {
    			path : '/'    
    		});
            /* 传是否是申请跟新的 */
            $.cookie("updateApply", JSON.stringify(true), {
    			path : '/'    
    		});
            /* 传报告类型*/
            $.cookie("updateReportType", JSON.stringify(root.reportType), {
    			path : '/'    
    		});
            //跳转到录入报告界面
            window.open('/fsn-core/views/market/'+root.reportPage);
        }else {//无权限：弹框提示用户无权限录报告
