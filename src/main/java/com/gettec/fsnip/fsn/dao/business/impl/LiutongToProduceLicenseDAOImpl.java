package com.gettec.fsnip.fsn.dao.business.impl;

import org.springframework.stereotype.Repository;

import com.gettec.fsnip.fsn.dao.business.LiutongToProduceLicenseDAO;
import com.gettec.fsnip.fsn.dao.common.impl.BaseDAOImpl;
import com.gettec.fsnip.fsn.model.business.LiutongToProduceLicense;

@Repository(value="liutongToProduceLicenseDAO")
public class LiutongToProduceLicenseDAOImpl extends BaseDAOImpl<LiutongToProduceLicense> implements
		LiutongToProduceLicenseDAO {

}
